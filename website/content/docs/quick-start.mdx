...
The fastest way to experiment with Hydroxide is our interactive playground. Write code and see the compiled output in real-time-no setup required.
...
export function App() {
  ...
  return (
    <button on-click={() => count.do(v => v + 1)}>
      Count: {count()}
    </button>
  )
}
